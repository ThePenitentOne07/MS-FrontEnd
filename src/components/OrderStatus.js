...
const OrderStatus = () => {
  ...
  const statusToStepIndexMap = {
    ...
    PAID: 0,
    IN_DELIVERY: 1,
    PREORDERED_ORDER_IN_DELIVERY: 1,
    CANNOT_DELIVER: 1,
    CANNOT_CONFRIRM: 1,
    COMPLETE_EXCHANGE: 2,
    IS_FEEDBACK: 2,
  ...
  const statusDisplayMap = {
    PAID: "Đang xác nhận",
    PRE_ORDERED: "Đã đặt trước",
    IN_DELIVERY: "Đang giao",
    PREORDERED_ORDER_IN_DELIVERY: "Đơn đặt trước đang giao",
      ...
      <Box sx={{ mt: 5 }}>
        <Paper elevation={3} sx={{ p: 3 }}>
          ...
          <Typography variant="h6">
            Tên người nhận: <b>{order.receiverName}</b>
          </Typography>
          <Typography variant="h6">
            Tình trạng: <b>{displayedStatus}</b>
          </Typography>
          <Typography variant="h6">
          ...
          {order.orderStatus === "CANNOT_DELIVER" && (
            <>
              <Typography variant="h6" sx={{ color: "red" }}>
                Lý do tạm hoãn:{" "}
                <b>{order.failureReasonNote.split(";")[1].split("|")[0]}</b>
              </Typography>
              <Typography variant="h6" sx={{ color: "red" }}>
                Ngày:{" "}
                <b>
                  {
                    order.failureReasonNote
                      .split(";")[1]
                      .split("|")[1]
                      .split("T")[0]
                  }
          ...
          {order.orderStatus === "CANNOT_CONFRIRM" && (
            <>
              ...
              <Typography variant="h6" sx={{ color: "red" }}>
                Ngày:{" "}
                <b>{order.failureReasonNote.split("|")[1].split("T")[0]}</b>
              </Typography>
              <Typography variant="h6" sx={{ color: "red" }}>
                Giờ:{" "}
                <b>
