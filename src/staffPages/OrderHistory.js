...
import { Stack, Rating, Container, Typography, Box, Dialog, DialogActions, DialogContent, DialogTitle, Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, MenuItem, Select, FormControl } from '@mui/material';
...
const OrderHistory = () => {
    ...
    const handleRowClick = async (order) => {
        if (order.orderStatus !== 'IN_CART') {
            ...
            try {
                ...
                try {
                    const response = await apiService.get(`/api/feedbacks/${order.id}`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem("token")}`
                        }
                    });
                    setFeedback(response.data.result);
                    console.log("Feedback data:", response.data.result.description);
                } catch (innerError) {
                    console.error("Failed to fetch feedback", innerError);
                }
            } catch (error) {
    ...
    const getOrders = async () => {
        try {
            ...
            setOrders(res.data.result.filter(order => order.orderStatus == "IS_FEEDBACK" || order.orderStatus == 'COMPLETE_EXCHANGE'));
        } catch (error) {
            ...
            {selectedOrderStatus && (
                <Dialog open={true} onClose={handleClose} maxWidth="lg">
                    ...
                    <DialogContent>
                        ...
                        {orderDetail.orderStatus === 'IS_FEEDBACK' && (
                            <>

                                <Typography>Customer feedback: {feedback.description}</Typography>
                                <Box display="flex" alignItems="center">
                                    <Typography sx={{ fontSize: '1.25rem', marginRight: '0.5rem' }}>Rating: {feedback.rating}</Typography>
                                    <StarRateIcon />
                                </Box>
                            </>
                        )}
